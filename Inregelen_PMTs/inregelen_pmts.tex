...
\begin{tikzpicture}[remember picture, overlay,
  titlebox/.append style={seccolor, fill, text=white, minimum height=3em,
    font=\sffamily\Huge},
  authorbox/.append style={minimum height=2em, font=\sffamily}]

  \node[titlebox, anchor=north west, shift={(2cm, -1cm)}] at
    (current page.north west) {Theorie};

  \node[titlebox, anchor=north east, shift={(-1cm, -1cm)}] at
    (current page.north east) {\thetitle};

  \node[authorbox, anchor=north east, shift={(-1cm, -2cm)}] at
    (current page.north east) {\theauthor};

  \draw[seccolor] ($(current page.north east) + (-1cm, -.5cm)$) --
    ($(current page.south east) + (-1cm, .5cm)$);
 
  \fill[seccolor] ($(current page.north east) + (-.5cm, -2cm-\thepage*1cm)$)
  rectangle +(-.5cm, +.5cm);
\end{tikzpicture}
}
\fancyhead{\thumb}
