...
\uplevel{Op de site \url{data.hisparc.nl} zie je een overzicht van de
...
geeft dus aan dat er een kosmische shower is gedetecteerd.} \uplevel{De
metingen van het aantal showers wat per uur gemeten is staat uitgezet in
het event histogram, zie \figref{fig:Event_histogram}} \uplevel{Soms zie
je dat het aantal events (dus  constant over de uren van de dag, maar er
zijn ook stations die een afwijkende hoeveelheid events laten zien.}
\uplevel{Die afwijkingen in het event histogram komen dan ook vaker
...
\uplevel{Probeer met behulp van de dataretrieval tool op tijdstippen dat
...
station etc.} \question{ Waardoor kunnen we het afwijkende event
histogram verklaren?}
